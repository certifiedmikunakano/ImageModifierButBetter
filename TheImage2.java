...
public class TheImage2 {
    ...
    private int [] safePrimes = {5, 7, 11, 23, 47, 59, 83, 107, 167, 179, 227, 263, 347, 359, 383, 467, 479, 503, 563, 587, 719, 839, 863, 887, 983, 1019, 1187, 1283, 1307, 1319};
    ...
    public TheImage2 (BufferedImage im) {
   	 rawImage = im;
   	percentEncrypted = 0;
  	 percentDecrypted = 0;
  	 percentEmojified = 0;
	...
	static int discreteLogarithm(int a, int b, int m)
	{
 	...
 	//use pr1
   	...
   	private static BufferedImage mosaicScramble (BufferedImage original, int pr) throws IOException {
		  ...
		  Mosaic m = new Mosaic (safePrime - 1, pixelSize, scrambled);
		  //scramble each row
...
public static BufferedImage mosaicUnscrambleRows (BufferedImage original, int pr) throws IOException {
	  ...
	  Mosaic m = new Mosaic (safePrime - 1, pixelSize, unscrambled);
...
class Mosaic {
		...
		public Mosaic (int spd, int ps, BufferedImage b) throws IOException {
			dimension = spd;
			pixelSize = ps;
			orig = b;
			fractionate();
